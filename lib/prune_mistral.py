...
def compute_bi(args, model, tokenizer, device=torch.device("cuda:0")):
    ...
    for i in range(len(layers)):
        layer = layers[i]
        subset = find_layers(layer, layers=[MistralDecoderLayer])
        ...
        if bi_score[i] < 1:
            for name in subset:
                ...
                if name in ['mlp.down_proj']:
                    ...
                    d_int = cov.shape[0]
                    ridge_inv = torch.linalg.inv(cov + torch.eye(d_int, device=device))
                    ...
                    subset['mlp.down_proj'].weight = torch.nn.Parameter(W2.to(torch.bfloat16))
                    subset['mlp.up_proj'].weight = torch.nn.Parameter(W1.to(torch.bfloat16))
                    subset['mlp.gate_proj'].weight = torch.nn.Parameter(W0.to(torch.bfloat16))
