...
def compute_bi_qwen(args, model, tokenizer, device=torch.device("cuda:0")):
    use_cache = model.config.use_cache 
    model.config.use_cache = False 
    ...
    for i in range(len(layers)):
        layer = layers[i]
        subset = find_layers(layer, layers=[Qwen2DecoderLayer, Qwen3DecoderLayer])
...
@torch.no_grad()
def prune_flatllm_qwen(args, model, tokenizer, device=torch.device("cuda:0")):
    use_cache = model.config.use_cache 
    model.config.use_cache = False 
